    ...
    <header className={`w-full fixed top-0 left-0 z-50 shadow-sm bg-white transition-transform duration-500 ${isVisible ? 'translate-y-0' : '-translate-y-full'}`}>
      <div className="py-4">
        <div className="custom-container">
          <div className="flex items-center justify-between">
            ...
            {/* Desktop Navigation */}
            <nav className="hidden xl:flex items-center gap-6 xl:gap-10 lato-font">
              {navLinks.map((link) => (
                <Link
                  key={link.name}
                  href={link.href}
                  className={`header-link transition-colors py-2 whitespace-nowrap lato ${link.active
                    ? "text-[#000000] font-bold"
                    : "text-[#000000] hover:text-[#E30613] font-medium hover:font-bold hover:underline hover:underline-offset-3"
                    }`}
                >
                  {link.name}
                </Link>
      ...
      {isMobileMenuOpen && (
        <div className="xl:hidden absolute top-full left-0 w-full bg-white shadow-lg flex flex-col z-50 border-t border-gray-100">
          <nav className="flex flex-col py-2 lato-font">
            {navLinks.map((link) => (
              <Link
                key={link.name}
                href={link.href}
              ...
              >
                <div className="custom-container py-4">
                  <span className={`header-link ${link.active
                    ? "text-[#E30613] font-bold lato"
                    : "text-[#333333] hover:text-[#E30613] font-semibold lato"
                    }`}>
                    {link.name}
                  </span>
                </div>
            ...
            <div className="custom-container py-6">
              <Button href="#" variant="primary" className="w-full flex justify-center items-center btn-text">
                Request Quote
              </Button>
            </div>
          </nav>
