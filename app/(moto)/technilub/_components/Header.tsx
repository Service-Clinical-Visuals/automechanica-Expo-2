...
export default function Header() {
  ...
  return (
    ...
    <header
      className={`w-full fixed top-0 left-0 z-50 shadow-sm bg-secondary transition-transform duration-500 ${isVisible ? "translate-y-0" : "-translate-y-full"}`}>
      <Container>
          ...
          <nav className="hidden lg:flex items-center gap-1">
            {navLinks.map((link) => (
              <button
                ...
                {link.label}
                {link.hasDropdown && <MdArrowDropDown />}
              </button>
            ))}
          </nav>
