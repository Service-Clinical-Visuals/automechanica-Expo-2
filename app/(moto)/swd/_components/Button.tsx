...
const Button = ({
  ...
  const variantStyles = 
    variant === "primary" ? "bg-[#2D3B6F] text-white hover:bg-[#1F294F]" :
    variant === "outline" ? "bg-transparent text-white border border-white/60 hover:bg-white/10 hover:border-white" :
    "bg-white text-[#2D3B6F] hover:bg-gray-100";

  const content = (
    <div className={`${baseStyles} ${variantStyles} ${className}`}>
      <span className="button whitespace-nowrap font-secondary">
        {text}
      </span>
      {showIcon && (
        <ArrowRightCircle 
          className={`ml-3 w-5 h-5 min-[2000px]:w-6 min-[2000px]:h-6 group-hover:translate-x-1 transition-transform duration-300 ${variant === 'secondary' ? 'text-[#2D3B6F]' : 'text-white'}`} 
          strokeWidth={1.75} 
        />
      )}
    </div>
