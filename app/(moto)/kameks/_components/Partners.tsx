      ...
      <div className="custom-container">
        ...
        <div
          ...
          <p className="inter-font text-[#404040] section-text max-w-7xl mx-auto leading-relaxed">
            High-performance automotive products engineered for reliability,
            durability, & everyday efficiency. From essential components to
            advanced solutions, our range is designed to support every vehicle &
            every journey.
          </p>
        </div>
        ...
        {/* Carousel / Grid Area */}
        <div
          className="relative flex items-center justify-between h-full w-full"
          data-aos="fade-up"
          data-aos-delay="200">
          {/* Left Arrow */}
          <button
          ...
          <div className="overflow-hidden w-full mx-auto">
            <div
              ...
              {chunkedProducts.map((chunk, slideIndex) => (
                <div
                  key={slideIndex}
                  className="min-w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                  {chunk.map((product, index) => (
                    <div
                      key={index}
                      className="group flex flex-col bg-white border-1 border-[#CCCCCC] rounded-[8px] transition-all duration-300 hover:border-[#F12535] hover:shadow-lg relative overflow-hidden">
                      {/* Badge */}
                      ...
                      <div className="flex flex-col items-start pt-6 pb-6 px-5 transition-all duration-300">
                        <div className="flex items-center gap-2 mb-3">
                          ...
                          <h3 className="poppins-font font-semibold text-[15px]">
                            <span className="text-[#F12535] font-semibold product-title">
                              Modal No :{" "}
                            </span>
                            <span className="text-[#202020] font-semibold product-title">
                              {product.model}
                            </span>
                          </h3>
                        </div>

                        <div className="inter-font font-semibold card-title text-[#202020] mb-2">
                          OEM :{" "}
                          <span className="text-[#404040] font-semibold card-title">
                            {product.oem}
                          </span>
                        </div>
                        <div className="inter-font font-semibold card-title text-[#202020] mb-5">
                          Material :{" "}
                          <span className="font-semibold card-title text-[#404040]">
                            {product.material}
                          </span>
                        </div>
                        ...
                        <Button
                          href="#"
                          variant="outline"
                          className="w-full py-2.5 rounded-[6px] !border-[1px] !border-[#F12535] !text-[#F12535] font-semibold text-xs poppins-font bg-transparent hover:!bg-[#F12535] hover:!text-white transition-colors flex items-center justify-center">
                          View Product
                        </Button>
                      </div>
                    </div>
          ...
          <button
            onClick={() =>
              setCurrentSlide((prev) =>
                Math.min(chunkedProducts.length - 1, prev + 1),
              )
            }
