      ...
      <div className="py-4">
        <div className="custom-container">
          <div className="flex items-center justify-between">
            ...
            {/* Desktop Navigation */}
            <nav className="hidden xl:flex items-center gap-8 xl:gap-12 inter-font">
              {navLinks.map((link) => (
                <Link
                  key={link.name}
                  href={link.href}
                  className={`header-link transition-colors py-2 whitespace-nowrap ${link.active
                    ? "header-link text-[#282361] font-medium underline underline-offset-4"
                    : "header-link text-[#404040] hover:text-[#282361] font-regular"
                    }`}
                >
                  {link.name}
                </Link>
